...
export function LoginPage() {
  ...
  async function handleForgot(formEvent: React.FormEvent) {
    formEvent.preventDefault();
    setForgotError('');
  ...
  // ── Left panel — shared across all modes ─────────────────────────────────
  const leftPanel = (
    <div className="hidden lg:flex lg:w-[45%] bg-navy-deep flex-col justify-between p-12 relative overflow-hidden select-none">
      ...
      <div className="absolute inset-0 opacity-[0.06] pointer-events-none">
        <svg width="100%" height="100%">
          <defs>
            <pattern id="t-grid" x="0" y="0" width="48" height="48" patternUnits="userSpaceOnUse">
      ...
      <div className="relative z-10">
        ...
        <p className="text-white/55 text-sm leading-relaxed mb-8 max-w-xs">
          Turn RFPs into structured bid plans — AI-extracted requirements, compliance tracking, and evidence-backed decisions in one place.
        </p>

        <p className="text-white/35 text-[11px] font-semibold uppercase tracking-widest mb-4">
          Built to help bid teams answer
        </p>
        <div className="flex flex-col">
          {[
            'Are we compliant with every requirement?',
            "What's our readiness score?",
            'Should we bid or walk away?',
          ].map((question, index) => (
            <div
              key={index}
              className="flex items-center gap-3 py-3 border-t border-white/10 last:border-b last:border-white/10"
            >
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="email"
                    value={forgotEmail}
                    onChange={forgotEmailEvent => setForgotEmail(forgotEmailEvent.target.value)}
