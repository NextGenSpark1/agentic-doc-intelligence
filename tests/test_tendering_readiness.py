"""Submission readiness — the gap analysis and scoring.
...
Written against the workspace model: requirements carry `req_id`, `mandatory`, `status`
(met/gap/partial/unchecked) and `owner`; evidence links carry `req_id`, `doc_id` and
`human_review_status`. There is no dismissed state for a requirement, no task list and no
superseded-document rule — the fixtures below are the current contract, and tests for those
removed features were deleted rather than adapted.
"""
...
def _requirement(req_id="REQ-1", mandatory=True, status="gap", completion="not_started",
                 owner="user-1"):
    """A requirement nobody has marked met — satisfaction has to come from evidence."""
    return {
        "req_id": req_id,
...
def _link(req_id="REQ-1", doc="LIB-1", status="confirmed"):
    return {"id": f"EVL-{req_id}-{doc}", "req_id": req_id, "doc_id": doc,
            "human_review_status": status, "score": 0.9}


def _library_doc(doc="LIB-1", expiry=None):
    return {"doc_id": doc, "title": "CIDB Certificate", "expiry_date": expiry}


def _workspace(closing="2026-10-01"):
    return {"id": "ws-1", "org_id": "org-1", "closing_date": closing}
...
def test_status_met_satisfies_without_evidence():
    """A person can mark a requirement met — that decision stands on its own."""
    assert is_satisfied(_requirement(status="met"), [], {}, TODAY) is True
...
def test_score_of_an_empty_workspace_is_zero_not_one():
    """No requirements means unknown, not perfect — an empty workspace must never read as ready."""
...
def test_dismissed_evidence_link_does_not_count():
    result = compute_score([_requirement()], [_link(status="dismissed")], [_library_doc()], TODAY)
    assert result["score"] == 0.0


# ------------------------------- gaps -------------------------------
def test_unmet_mandatory_requirement_is_a_blocker():
    gaps = compute_gaps(_workspace(), [_requirement()], [], [_document()], [], TODAY)
    blocker = next(g for g in gaps if g["gap_type"] == "mandatory_requirement_unmet")
    assert blocker["severity"] == "blocker"
    assert blocker["req_id"] == "REQ-1"


def test_unmet_optional_requirement_is_only_a_warning():
    gaps = compute_gaps(_workspace(), [_requirement(mandatory=False)], [], [_document()], [], TODAY)
...
def test_evidence_expiring_before_the_closing_date_is_a_blocker():
    ...
    gaps = compute_gaps(
        _workspace(closing="2026-12-01"), [_requirement()], [_link()], [_document()],
        [_library_doc(expiry="2026-11-01")], TODAY,
    )
...
def test_evidence_valid_past_the_closing_date_is_not_flagged():
    gaps = compute_gaps(
        _workspace(closing="2026-10-01"), [_requirement()], [_link()], [_document()],
...
def test_already_expired_evidence_is_a_blocker():
    gaps = compute_gaps(_workspace(), [_requirement()], [_link()], [_document()],
                        [_library_doc(expiry="2026-01-01")], TODAY)
    assert "evidence_expired" in _gap_types(gaps)


def test_evidence_expiring_within_the_warning_window():
    gaps = compute_gaps(_workspace(closing="2026-09-10"), [_requirement()], [_link()],
                        [_document()], [_library_doc(expiry="2026-09-20")], TODAY)
    gap = next(g for g in gaps if g["gap_type"] == "evidence_expiring_soon")
    assert gap["severity"] == "warning"


def test_pending_evidence_on_a_mandatory_requirement_blocks():
    gaps = compute_gaps(_workspace(), [_requirement()], [_link(status="pending")], [_document()],
                        [_library_doc()], TODAY)
...
def test_unreviewed_requirement_is_flagged():
    gaps = compute_gaps(_workspace(), [_requirement(status="unchecked")], [_link()], [_document()],
                        [_library_doc()], TODAY)
    assert "requirement_pending_review" in _gap_types(gaps)


def test_passed_closing_date_is_a_blocker():
    gaps = compute_gaps(_workspace(closing="2026-08-01"), [_requirement()], [_link()],
                        [_document()], [_library_doc()], TODAY)
    gap = next(g for g in gaps if g["gap_type"] == "closing_date_passed")
    assert gap["severity"] == "blocker"


def test_imminent_closing_date_is_a_warning():
    gaps = compute_gaps(_workspace(closing="2026-09-06"), [_requirement()], [_link()],
                        [_document()], [_library_doc()], TODAY)
    assert "closing_date_imminent" in _gap_types(gaps)


def test_missing_closing_date_is_flagged():
    gaps = compute_gaps(_workspace(closing=None), [_requirement()], [_link()], [_document()],
                        [_library_doc()], TODAY)
...
def test_failed_document_extraction_blocks():
    ...
    gaps = compute_gaps(_workspace(), [_requirement()], [_link()], [_document(status="failed")],
                        [_library_doc()], TODAY)
...
def test_queued_document_is_a_warning_not_a_blocker():
    gaps = compute_gaps(_workspace(), [_requirement()], [_link()], [_document(status="queued")],
                        [_library_doc()], TODAY)
    gap = next(g for g in gaps if g["gap_type"] == "document_not_processed")
    assert gap["severity"] == "warning"
...
def test_blockers_are_listed_first():
    ...
    report = build_report(_workspace(), [_requirement(owner="")], [], [_document()], [], TODAY)
...
def test_a_high_score_can_still_be_blocked():
    ...
    report = build_report(_workspace(), requirements, links, [_document()], library, TODAY)
...
def test_deterministic_narrative_states_blocked_plainly():
    report = build_report(_workspace(), [_requirement()], [], [_document()], [], TODAY)
